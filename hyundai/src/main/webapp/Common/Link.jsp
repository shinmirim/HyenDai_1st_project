...
<header id="header">
    ...
    <div class="inner">
        <!-- toparea// -->
        <div class="toparea">
            <h1><a href="TohomeServlet?command=main" onclick="location.href='TohomeServlet?command=main'"><img
                    src="./UIUX/images/header_logo_freex34.png" alt="현대식품관"></a></h1>

<!-- 로그인 여부에 따른 메뉴 변화 -->
          <% if (session.getAttribute("UserId") == null) { %>
           <!-- util : 로그인 전// -->
              <div class="util" id="dawnLoginN" style="display: hidden">
                  <a href="TohomeServlet?command=login_form" onclick="GA_Event('PC_공통', '헤더', '로그인'); javascript:loginPopup('');return false;">로그인</a>
                  <a href="TohomeServlet?command=join_form"  onclick="GA_Event('PC_공통', '헤더', '회원가입');">회원가입</a>
           ...
           <% } else { %>
			<!-- util : 로그인 후// -->
               <div class="util" id="dawnLoginY" style="display: hidden">
                   <button type="button" class="btn-linedown btn-personal" id="memNmText" > <%=session.getAttribute("UserName") %> 님! 반갑습니다.</button>
                   <a href="TohomeServlet?command=logout" class="active">로그아웃</a><!-- 활성화 class="active" 추가 -->
                   <!-- <a href="">회원정보변경</a> -->
                   <a href="TohomeServlet?command=my_page" onclick="GA_Event('PC_공통', '헤더', '마이페이지');">마이페이지</a>
        ...
        <nav class="gnbarea">
            ...
            <div id="popCategory">
              ...
              function category_onclick(){
                ...
                <!-- category// -->
                <button type="button" class="btn-category" onclick = "category_onclick()">카테고리리 전체보기</button>
            ...
            <ul class="gnb-list" id="homeGnbList">
                <li><a href="TohomeServlet?command=latest_productList&top_num=0&filter=0" style="color : #ff6913 ">신상품</a>
                </li>
                <li><a href="TohomeServlet?command=all_productList&top_num=0&filter=0">전체상품</a>
                </li>
            </ul>
            <!-- //gnb -->
            ...
            <button type="button" class="btn-cart"
                    onclick="location='TohomeServlet?command=basket'">장바구니
                <%--TODO :: DB에서 user_no별 basket의 행의 갯수 가져오기--%>
                <span id="basketCnt"><% 
                int count = 0;
                if(session.getAttribute("basketCount")!= null){count = (int)session.getAttribute("basketCount");}
                out.print(count);%></span></button>
        </nav>
        <!-- //gnbarea -->
    </div>
</header>
